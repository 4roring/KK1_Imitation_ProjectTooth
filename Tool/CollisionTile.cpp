...
HRESULT CCollisionTile::Initialize()
{
	...
	for (int y = 0; y < ISOTILEY; ++y)
	{
		...
		{
			COLLITILE* pIsoTile = new COLLITILE;
...
void CCollisionTile::Update()
{
	m_iPickIndex = GetTileIndex(GetMousePos());
	Picking();
}

void CCollisionTile::Render()
{
	...
	for (int y = 0; y < ISOTILEY; ++y)
	{
		for (int x = 0; x < ISOTILEX; ++x)
		{
			...
			pTexture = TextureMgr->GetInstance()->GetTexture(
				TEXT("TILE"), TEXT("CollisionTile"), m_vecIsoTile[iIndex]->byDrawID);
...
void CCollisionTile::Release()
{
	std::for_each(m_vecIsoTile.begin(), m_vecIsoTile.end(), SafeDelete<COLLITILE*>);
	VECCOLLTILE vec;
	m_vecIsoTile.swap(vec);
}

void CCollisionTile::MiniMap()
{
	const TEXINFO* pTexture = TextureMgr->GetTexture(TEXT("Map03"));
...
bool CCollisionTile::isCulling(const D3DXVECTOR3& vPos)
{
	float fZoom = ToolMgr->GetZoom();
	D3DXVECTOR3 vScroll = ToolMgr->GetScroll();
...
D3DXVECTOR3 CCollisionTile::GetMousePos()
{
	float fZoom = ToolMgr->GetZoom();
	D3DXVECTOR3 vScroll = ToolMgr->GetScroll();
...
int CCollisionTile::GetTileIndex(D3DXVECTOR3 vPos)
{
...
void CCollisionTile::TileChange()
{
}
